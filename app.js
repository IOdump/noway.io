...
function checkAuth (req, res, next) {
  ...
  if (req.query.apikey != app.get('api key') && req.path.substring(0, 6) != "/osrm/"  ) {
...
app.put('/profiles/:profileid', function(req, res){
  ...
  client.exists('profiles:' + profileid, function (err, exists) {
      ...
      if (exists == 0) {
          ...
          res.end(JSON.stringify({created: 'ko, does not already exists'}));
...
app.get('/ports/:profileid', function(req, res){
    var profileid = req.params.profileid;

    client.exists('ports:' + profileid, function (err, exists) {
      console.log(exists);
      ...
      if (exists == 0) {
        ...
        client.get('ports:' + profileid, function (err, data) {
            res.end(data);
        });
